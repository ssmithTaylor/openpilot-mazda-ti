...
FrogPilotLateralPanel::FrogPilotLateralPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> lateralToggles {
    ...
    {"LatDemandCap", tr("Cap Steering Demand"), tr("<b>Never ask the steering for more than it can deliver.</b> In a corner beyond the car's limit, openpilot keeps asking harder anyway. The extra request buys nothing — the car is already giving everything — but it pins the command flat at its ceiling, and a pinned command is when the steering jams mid-corner: on recorded drives, essentially every mid-corner jam happened while pinned. This caps the request at what the car can actually do, so the command keeps moving and the steering stays responsive at its limit. It only ever asks for less, never more, and is safe to switch while driving."), ""},
    {"LatFFLookahead", tr("Start Steering Earlier"), tr("<b>Ask for the corner when it is coming, not when it arrives.</b> This car's steering takes most of a second between being asked and actually delivering, so torque requested at the corner shows up late and the car enters behind. The driving model already knows the road nearly a second ahead; this steers from that preview, so the torque lands on time. It moves steering earlier, never asks for more of it — the same limits still bound everything — and it fades in and out over a second, so it is safe to switch while driving."), ""},
  ...
  QSet<QString> liveUpdateKeys = {"LatOutputFilter", "LatNoFrictionRelay", "LatStallModulation",
                                  "LatDemandCap", "LatFFLookahead", "LatFrictionComp",
