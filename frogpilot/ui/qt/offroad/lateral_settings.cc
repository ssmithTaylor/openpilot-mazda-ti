...
FrogPilotLateralPanel::FrogPilotLateralPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> lateralToggles {
    ...
    {"LatFrictionComp", tr("Compensate Steering Friction"), tr("<b>Add back the torque the steering's own friction eats.</b> Measured across four drives — warm, cold, and wet — the rack loses a steady 30 to 80 counts of every command to internal friction, which is why openpilot always runs slightly behind what it asks for and leans on slow trim to make up the difference. This adds that measured amount in the direction it is already steering. It fades to nothing on straight road and near maximum steering, and everything stays inside the same limits as before. Safe to switch while driving."), ""},
    {"LatCommitSetpoint", tr("Hold the Corner Line"), tr("<b>Stop one wobble from talking the steering out of a corner.</b> In hard corners the steering sticks, breaks free with a small overshoot, and the plan then eases off in sympathy with the car's own swing — which unloads the wheel and lets it stick again, over and over. This makes the target the car steers toward quick to ask for more but slow to ask for less, so a single overshoot cannot unwind the corner. It only acts in real corners, never on straights, is bounded to what the plan is asking plus a small margin, and lets go immediately when the corner direction changes. Safe to switch while driving."), ""},
    {"LatDamping", tr("Damp Steering Swings"), tr("<b>Absorb the snap when stuck steering lets go.</b> In hard corners the steering sticks, then breaks free with a lurch past what was asked, then swings back — the sawtooth feel through long curves. This adds resistance proportional to how fast the car's turning is changing, which soaks up the lurch instead of letting it ring. It does nothing while the wheel is stuck, nothing at rest, and everything stays inside the same limits as before. Safe to switch while driving."), ""},
  ...
  QSet<QString> liveUpdateKeys = {"LatOutputFilter", "LatNoFrictionRelay", "LatFrictionComp", "LatCommitSetpoint", "LatDamping",
