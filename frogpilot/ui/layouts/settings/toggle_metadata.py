...
STEERING_TOGGLES = (
  ...
  ToggleDefinition(
    ...
    param="TiSteerMax",
    button_labels=["Reset"],
    description=("<b>The most steering effort openpilot can ask the interceptor for.</b> Lower this to cap how strong assist can get. The interceptor is documented to ignore anything above 600, but that has never actually been measured — 610 and 620 exist to test it."),
    max_value=620,
    min_value=100,
    parent_param="TorqueInterceptorTune",
    step=10,
    toggle_type=ToggleType.NUMERIC_WITH_BUTTON,
    tuning_level=3,
  ),
  ToggleDefinition(
    title=("TI Ramp-Up Rate"),
    param="TiSteerDeltaUp",
    button_labels=["Reset"],
    description=("<b>How quickly steering effort is allowed to build.</b> Raise for sharper response entering corners. Too high and the interceptor treats it as unsafe and cuts assist to zero. Default 6 takes one second to reach full."),
    max_value=30,
    min_value=1,
    parent_param="TorqueInterceptorTune",
    step=1,
  ...
  ToggleDefinition(
    title=("TI Ramp-Down Rate"),
    param="TiSteerDeltaDown",
    button_labels=["Reset"],
    description=("<b>How quickly steering effort is allowed to release.</b> Raise to hand control back faster when openpilot backs off; lower for smoother corner exits."),
    max_value=50,
    min_value=1,
    parent_param="TorqueInterceptorTune",
    step=1,
  ...
  ToggleDefinition(
    title=("TI Driver Torque Allowance"),
    param="TiSteerDriverAllowance",
    button_labels=["Reset"],
    description=("<b>How firmly you can hold the wheel before openpilot starts easing off.</b> Raise if assist fades just from resting a hand on the wheel; lower to hand over control sooner."),
    max_value=60,
    min_value=5,
    parent_param="TorqueInterceptorTune",
    step=1,
  ...
  ToggleDefinition(
    title=("TI Driver Torque Backoff"),
    param="TiSteerDriverMultiplier",
    button_labels=["Reset"],
    description=("<b>How sharply assist drops once you push past the allowance.</b> Lower it if steering gives up on you mid-corner; raise it to hand over control more readily. At the default of 40, assist is gone almost immediately."),
    max_value=60,
    min_value=1,
    parent_param="TorqueInterceptorTune",
    step=1,
  ...
  ToggleDefinition(
    title=("TI Steering Pressed Threshold"),
    param="TiSteerThreshold",
    button_labels=["Reset"],
    description=("<b>How much pressure on the wheel counts as you taking over.</b> Raise if bumps and road feedback falsely trigger a takeover; lower to have openpilot notice your input sooner."),
    max_value=40,
    min_value=1,
    parent_param="TorqueInterceptorTune",
    step=1,
